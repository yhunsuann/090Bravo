...
<form action="{{ route('admin.recruitment.deletes') }}" method="post">
    ...
    <div class="row">
        <div class="col col-table px-3 bg-white" style="overflow-x:auto;">
            <table class="table caption-top bg-white table table-striped table-hover">
                <thead>
                    <tr class="py-3">
                ...
                <tbody class="table-group-divider">
                    @forelse($recruitments as $item)
                        <tr>
                            ...
                            <td><img height="40px" src="{{ asset('assets/img/cruitments/' . $item->image)}}" alt=""></td>
                            <td>{{ $item->status }}</td>
                            <td>{{ $item->created_at}}</td>
                            <td class="text-center action-form">
                                <a data-id="{{ $item->id }}" data-coreui-toggle="modal" data-coreui-target="#exampleModal" class="open-modal"><i class="fa fa-solid fa-trash"></i></a> &nbsp;
