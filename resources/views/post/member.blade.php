...
<h4 class="mb-4">Information Member</h4>
<h5 class="title-create">Post member edit</h5>
    ...
    <div class="row image mb-2">
        ...
        @foreach ($result as $post)
            ...
            @foreach ($images as $image)
                @php
                    $allImages[] = $image[0]; // Thêm tên ảnh vào mảng allImages
                @endphp

                <div class="col-3 mt-2">
                    <input type="hidden" name="upload_image" class="images-list" value="{{ json_encode($allImages) }}"></input>
                    <img src="{{ asset('assets/img/post/'.$image[0]) }}" class="" alt="{{ $image[0] }}">
